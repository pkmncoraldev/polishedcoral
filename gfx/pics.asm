INCLUDE "constants.asm"


SECTION "Pic Pointers", ROMX

INCLUDE "data/pokemon/pic_pointers.asm"


SECTION "Variant Pic Pointers", ROMX

INCLUDE "data/pokemon/variant_pic_pointers.asm"


SECTION "Trainer Pic Pointers", ROMX

INCLUDE "data/trainers/pic_pointers.asm"


SECTION "Painting Pic Pointers", ROMX

INCLUDE "data/events/paintings/pic_pointers.asm"


SECTION "Pokemon Pics 1", ROMX

BulbasaurFrontpic:        INCBIN "gfx/pokemon/bulbasaur/front.2bpp.lz"
BulbasaurBackpic:         INCBIN "gfx/pokemon/bulbasaur/back.2bpp.lz"
IvysaurFrontpic:          INCBIN "gfx/pokemon/ivysaur/front.2bpp.lz"
IvysaurBackpic:           INCBIN "gfx/pokemon/ivysaur/back.2bpp.lz"
VenusaurFrontpic:         INCBIN "gfx/pokemon/venusaur/front.2bpp.lz"
VenusaurBackpic:          INCBIN "gfx/pokemon/venusaur/back.2bpp.lz"
CharmanderFrontpic:       INCBIN "gfx/pokemon/charmander/front.2bpp.lz"
CharmanderBackpic:        INCBIN "gfx/pokemon/charmander/back.2bpp.lz"
CharmeleonFrontpic:       INCBIN "gfx/pokemon/charmeleon/front.2bpp.lz"
CharmeleonBackpic:        INCBIN "gfx/pokemon/charmeleon/back.2bpp.lz"
CharizardFrontpic:        INCBIN "gfx/pokemon/charizard/front.2bpp.lz"
CharizardBackpic:         INCBIN "gfx/pokemon/charizard/back.2bpp.lz"
SquirtleFrontpic:         INCBIN "gfx/pokemon/squirtle/front.2bpp.lz"
SquirtleBackpic:          INCBIN "gfx/pokemon/squirtle/back.2bpp.lz"
WartortleFrontpic:        INCBIN "gfx/pokemon/wartortle/front.2bpp.lz"
WartortleBackpic:         INCBIN "gfx/pokemon/wartortle/back.2bpp.lz"


SECTION "Pokemon Pics 2", ROMX

BlastoiseFrontpic:        INCBIN "gfx/pokemon/blastoise/front.2bpp.lz"
BlastoiseBackpic:         INCBIN "gfx/pokemon/blastoise/back.2bpp.lz"
PidgeyFrontpic:           INCBIN "gfx/pokemon/pidgey/front.2bpp.lz"
PidgeyBackpic:            INCBIN "gfx/pokemon/pidgey/back.2bpp.lz"
Pidgey_RGFrontpic:        INCBIN "gfx/pokemon/pidgey_rg/front.2bpp.lz"
PidgeottoFrontpic:        INCBIN "gfx/pokemon/pidgeotto/front.2bpp.lz"
PidgeottoBackpic:         INCBIN "gfx/pokemon/pidgeotto/back.2bpp.lz"
PidgeotFrontpic:          INCBIN "gfx/pokemon/pidgeot/front.2bpp.lz"
PidgeotBackpic:           INCBIN "gfx/pokemon/pidgeot/back.2bpp.lz"
RattataFrontpic:          INCBIN "gfx/pokemon/rattata/front.2bpp.lz"
RattataBackpic:           INCBIN "gfx/pokemon/rattata/back.2bpp.lz"
Rattata_RGFrontpic:       INCBIN "gfx/pokemon/rattata_rg/front.2bpp.lz"


SECTION "Pokemon Pics 3", ROMX


RaticateFrontpic:         INCBIN "gfx/pokemon/raticate/front.2bpp.lz"
RaticateBackpic:          INCBIN "gfx/pokemon/raticate/back.2bpp.lz"
Raticate_RGFrontpic:      INCBIN "gfx/pokemon/raticate_rg/front.2bpp.lz"
RaichuFrontpic:           INCBIN "gfx/pokemon/raichu/front.2bpp.lz"
Raichu_RGFrontpic:        INCBIN "gfx/pokemon/raichu_rg/front.2bpp.lz"
RaichuBackpic:            INCBIN "gfx/pokemon/raichu/back.2bpp.lz"
SandshrewFrontpic:        INCBIN "gfx/pokemon/sandshrew/front.2bpp.lz"
SandshrewBackpic:         INCBIN "gfx/pokemon/sandshrew/back.2bpp.lz"
SandslashFrontpic:        INCBIN "gfx/pokemon/sandslash/front.2bpp.lz"
SandslashBackpic:         INCBIN "gfx/pokemon/sandslash/back.2bpp.lz"
NidoranFFrontpic:         INCBIN "gfx/pokemon/nidoran_f/front.2bpp.lz"
NidoranFBackpic:          INCBIN "gfx/pokemon/nidoran_f/back.2bpp.lz"
NidorinaFrontpic:         INCBIN "gfx/pokemon/nidorina/front.2bpp.lz"
NidorinaBackpic:          INCBIN "gfx/pokemon/nidorina/back.2bpp.lz"
NidoqueenFrontpic:        INCBIN "gfx/pokemon/nidoqueen/front.2bpp.lz"
NidoqueenBackpic:         INCBIN "gfx/pokemon/nidoqueen/back.2bpp.lz"


SECTION "Pokemon Pics 4", ROMX


NidoranMFrontpic:         INCBIN "gfx/pokemon/nidoran_m/front.2bpp.lz"
NidoranMBackpic:          INCBIN "gfx/pokemon/nidoran_m/back.2bpp.lz"
NidorinoFrontpic:         INCBIN "gfx/pokemon/nidorino/front.2bpp.lz"
NidorinoBackpic:          INCBIN "gfx/pokemon/nidorino/back.2bpp.lz"
NidokingFrontpic:         INCBIN "gfx/pokemon/nidoking/front.2bpp.lz"
NidokingBackpic:          INCBIN "gfx/pokemon/nidoking/back.2bpp.lz"
ClefairyFrontpic:         INCBIN "gfx/pokemon/clefairy/front.2bpp.lz"
ClefairyBackpic:          INCBIN "gfx/pokemon/clefairy/back.2bpp.lz"
ClefableFrontpic:         INCBIN "gfx/pokemon/clefable/front.2bpp.lz"
ClefableBackpic:          INCBIN "gfx/pokemon/clefable/back.2bpp.lz"
JigglypuffFrontpic:       INCBIN "gfx/pokemon/jigglypuff/front.2bpp.lz"
JigglypuffBackpic:        INCBIN "gfx/pokemon/jigglypuff/back.2bpp.lz"
WigglytuffFrontpic:       INCBIN "gfx/pokemon/wigglytuff/front.2bpp.lz"
WigglytuffBackpic:        INCBIN "gfx/pokemon/wigglytuff/back.2bpp.lz"
ZubatFrontpic:            INCBIN "gfx/pokemon/zubat/front.2bpp.lz"
ZubatBackpic:             INCBIN "gfx/pokemon/zubat/back.2bpp.lz"


SECTION "Pokemon Pics 5", ROMX


GolbatFrontpic:           INCBIN "gfx/pokemon/golbat/front.2bpp.lz"
GolbatBackpic:            INCBIN "gfx/pokemon/golbat/back.2bpp.lz"
OddishFrontpic:           INCBIN "gfx/pokemon/oddish/front.2bpp.lz"
OddishBackpic:            INCBIN "gfx/pokemon/oddish/back.2bpp.lz"
GloomFrontpic:            INCBIN "gfx/pokemon/gloom/front.2bpp.lz"
GloomBackpic:             INCBIN "gfx/pokemon/gloom/back.2bpp.lz"
VileplumeFrontpic:        INCBIN "gfx/pokemon/vileplume/front.2bpp.lz"
VileplumeBackpic:         INCBIN "gfx/pokemon/vileplume/back.2bpp.lz"
MeowthFrontpic:           INCBIN "gfx/pokemon/meowth/front.2bpp.lz"
MeowthBackpic:            INCBIN "gfx/pokemon/meowth/back.2bpp.lz"
PersianFrontpic:          INCBIN "gfx/pokemon/persian/front.2bpp.lz"
PersianBackpic:           INCBIN "gfx/pokemon/persian/back.2bpp.lz"
CacneaFrontpic:           INCBIN "gfx/pokemon/cacnea/front.2bpp.lz"
CacneaBackpic:            INCBIN "gfx/pokemon/cacnea/back.2bpp.lz"
CacturneFrontpic:         INCBIN "gfx/pokemon/cacturne/front.2bpp.lz"
CacturneBackpic:          INCBIN "gfx/pokemon/cacturne/back.2bpp.lz"


SECTION "Pokemon Pics 6", ROMX


GrowlitheFrontpic:        INCBIN "gfx/pokemon/growlithe/front.2bpp.lz"
GrowlitheBackpic:         INCBIN "gfx/pokemon/growlithe/back.2bpp.lz"
ArcanineFrontpic:         INCBIN "gfx/pokemon/arcanine/front.2bpp.lz"
ArcanineBackpic:          INCBIN "gfx/pokemon/arcanine/back.2bpp.lz"
PoliwagFrontpic:          INCBIN "gfx/pokemon/poliwag/front.2bpp.lz"
PoliwagBackpic:           INCBIN "gfx/pokemon/poliwag/back.2bpp.lz"
PoliwhirlFrontpic:        INCBIN "gfx/pokemon/poliwhirl/front.2bpp.lz"
PoliwhirlBackpic:         INCBIN "gfx/pokemon/poliwhirl/back.2bpp.lz"
PoliwrathFrontpic:        INCBIN "gfx/pokemon/poliwrath/front.2bpp.lz"
PoliwrathBackpic:         INCBIN "gfx/pokemon/poliwrath/back.2bpp.lz"
SunkernFrontpic:          INCBIN "gfx/pokemon/sunkern/front.2bpp.lz"
SunkernBackpic:           INCBIN "gfx/pokemon/sunkern/back.2bpp.lz"
SunfloraFrontpic:         INCBIN "gfx/pokemon/sunflora/front.2bpp.lz"
SunfloraBackpic:          INCBIN "gfx/pokemon/sunflora/back.2bpp.lz"
BarboachFrontpic:         INCBIN "gfx/pokemon/barboach/front.2bpp.lz"
BarboachBackpic:          INCBIN "gfx/pokemon/barboach/back.2bpp.lz"
WhiscashFrontpic:         INCBIN "gfx/pokemon/whiscash/front.2bpp.lz"
WhiscashBackpic:          INCBIN "gfx/pokemon/whiscash/back.2bpp.lz"


SECTION "Pokemon Pics 7", ROMX


MachopFrontpic:           INCBIN "gfx/pokemon/machop/front.2bpp.lz"
MachopBackpic:            INCBIN "gfx/pokemon/machop/back.2bpp.lz"
MachokeFrontpic:          INCBIN "gfx/pokemon/machoke/front.2bpp.lz"
MachokeBackpic:           INCBIN "gfx/pokemon/machoke/back.2bpp.lz"
MachampFrontpic:          INCBIN "gfx/pokemon/machamp/front.2bpp.lz"
MachampBackpic:           INCBIN "gfx/pokemon/machamp/back.2bpp.lz"
GeodudeFrontpic:          INCBIN "gfx/pokemon/geodude/front.2bpp.lz"
GeodudeBackpic:           INCBIN "gfx/pokemon/geodude/back.2bpp.lz"
GravelerFrontpic:         INCBIN "gfx/pokemon/graveler/front.2bpp.lz"
GravelerBackpic:          INCBIN "gfx/pokemon/graveler/back.2bpp.lz"
GolemFrontpic:            INCBIN "gfx/pokemon/golem/front.2bpp.lz"
GolemBackpic:             INCBIN "gfx/pokemon/golem/back.2bpp.lz"
PonytaFrontpic:           INCBIN "gfx/pokemon/ponyta/front.2bpp.lz"
PonytaBackpic:            INCBIN "gfx/pokemon/ponyta/back.2bpp.lz"
RapidashFrontpic:         INCBIN "gfx/pokemon/rapidash/front.2bpp.lz"
RapidashBackpic:          INCBIN "gfx/pokemon/rapidash/back.2bpp.lz"


SECTION "Pokemon Pics 8", ROMX


SlowpokeFrontpic:         INCBIN "gfx/pokemon/slowpoke/front.2bpp.lz"
SlowpokeBackpic:          INCBIN "gfx/pokemon/slowpoke/back.2bpp.lz"
SlowbroFrontpic:          INCBIN "gfx/pokemon/slowbro/front.2bpp.lz"
SlowbroBackpic:           INCBIN "gfx/pokemon/slowbro/back.2bpp.lz"
MagnemiteFrontpic:        INCBIN "gfx/pokemon/magnemite/front.2bpp.lz"
MagnemiteBackpic:         INCBIN "gfx/pokemon/magnemite/back.2bpp.lz"
MagnetonFrontpic:         INCBIN "gfx/pokemon/magneton/front.2bpp.lz"
MagnetonBackpic:          INCBIN "gfx/pokemon/magneton/back.2bpp.lz"
DoduoFrontpic:            INCBIN "gfx/pokemon/doduo/front.2bpp.lz"
DoduoBackpic:             INCBIN "gfx/pokemon/doduo/back.2bpp.lz"
DodrioFrontpic:           INCBIN "gfx/pokemon/dodrio/front.2bpp.lz"
DodrioBackpic:            INCBIN "gfx/pokemon/dodrio/back.2bpp.lz"
GrimerFrontpic:           INCBIN "gfx/pokemon/grimer/front.2bpp.lz"
GrimerBackpic:            INCBIN "gfx/pokemon/grimer/back.2bpp.lz"
MukFrontpic:           	  INCBIN "gfx/pokemon/muk/front.2bpp.lz"
MukBackpic:            	  INCBIN "gfx/pokemon/muk/back.2bpp.lz"
GastlyFrontpic:           INCBIN "gfx/pokemon/gastly/front.2bpp.lz"
GastlyBackpic:            INCBIN "gfx/pokemon/gastly/back.2bpp.lz"
HaunterFrontpic:          INCBIN "gfx/pokemon/haunter/front.2bpp.lz"
HaunterBackpic:           INCBIN "gfx/pokemon/haunter/back.2bpp.lz"


SECTION "Pokemon Pics 9", ROMX


GengarFrontpic:           INCBIN "gfx/pokemon/gengar/front.2bpp.lz"
GengarBackpic:            INCBIN "gfx/pokemon/gengar/back.2bpp.lz"
OnixFrontpic:             INCBIN "gfx/pokemon/onix/front.2bpp.lz"
OnixBackpic:              INCBIN "gfx/pokemon/onix/back.2bpp.lz"
VoltorbFrontpic:          INCBIN "gfx/pokemon/voltorb/front.2bpp.lz"
VoltorbBackpic:           INCBIN "gfx/pokemon/voltorb/back.2bpp.lz"
ElectrodeFrontpic:        INCBIN "gfx/pokemon/electrode/front.2bpp.lz"
ElectrodeBackpic:         INCBIN "gfx/pokemon/electrode/back.2bpp.lz"
ExeggcuteFrontpic:        INCBIN "gfx/pokemon/exeggcute/front.2bpp.lz"
ExeggcuteBackpic:         INCBIN "gfx/pokemon/exeggcute/back.2bpp.lz"
Exeggcute_AFrontpic:      INCBIN "gfx/pokemon/exeggcute_a/front.2bpp.lz"
Exeggcute_ABackpic:       INCBIN "gfx/pokemon/exeggcute_a/back.2bpp.lz"
ExeggutorFrontpic:        INCBIN "gfx/pokemon/exeggutor/front.2bpp.lz"
ExeggutorBackpic:         INCBIN "gfx/pokemon/exeggutor/back.2bpp.lz"
Exeggutor_AFrontpic:      INCBIN "gfx/pokemon/exeggutor_a/front.2bpp.lz"
Exeggutor_ABackpic:       INCBIN "gfx/pokemon/exeggutor_a/back.2bpp.lz"
Exeggutor_RGFrontpic:     INCBIN "gfx/pokemon/exeggutor_rg/front.2bpp.lz"
CuboneFrontpic:           INCBIN "gfx/pokemon/cubone/front.2bpp.lz"
CuboneBackpic:            INCBIN "gfx/pokemon/cubone/back.2bpp.lz"
MarowakFrontpic:          INCBIN "gfx/pokemon/marowak/front.2bpp.lz"
MarowakBackpic:           INCBIN "gfx/pokemon/marowak/back.2bpp.lz"


SECTION "Pokemon Pics 10", ROMX


KoffingFrontpic:          INCBIN "gfx/pokemon/koffing/front.2bpp.lz"
KoffingBackpic:           INCBIN "gfx/pokemon/koffing/back.2bpp.lz"
WeezingFrontpic:          INCBIN "gfx/pokemon/weezing/front.2bpp.lz"
WeezingBackpic:           INCBIN "gfx/pokemon/weezing/back.2bpp.lz"
ChanseyFrontpic:          INCBIN "gfx/pokemon/chansey/front.2bpp.lz"
ChanseyBackpic:           INCBIN "gfx/pokemon/chansey/back.2bpp.lz"
DreepyFrontpic:           INCBIN "gfx/pokemon/dreepy/front.2bpp.lz"
DreepyBackpic:            INCBIN "gfx/pokemon/dreepy/back.2bpp.lz"
DrakloakFrontpic:         INCBIN "gfx/pokemon/drakloak/front.2bpp.lz"
DrakloakBackpic:          INCBIN "gfx/pokemon/drakloak/back.2bpp.lz"
StaryuFrontpic:           INCBIN "gfx/pokemon/staryu/front.2bpp.lz"
StaryuBackpic:            INCBIN "gfx/pokemon/staryu/back.2bpp.lz"
StarmieFrontpic:          INCBIN "gfx/pokemon/starmie/front.2bpp.lz"
StarmieBackpic:           INCBIN "gfx/pokemon/starmie/back.2bpp.lz"


SECTION "Pokemon Pics 11", ROMX

ScytherFrontpic:          INCBIN "gfx/pokemon/scyther/front.2bpp.lz"
ScytherBackpic:           INCBIN "gfx/pokemon/scyther/back.2bpp.lz"
ElectabuzzFrontpic:       INCBIN "gfx/pokemon/electabuzz/front.2bpp.lz"
ElectabuzzBackpic:        INCBIN "gfx/pokemon/electabuzz/back.2bpp.lz"
MagmarFrontpic:           INCBIN "gfx/pokemon/magmar/front.2bpp.lz"
MagmarBackpic:            INCBIN "gfx/pokemon/magmar/back.2bpp.lz"
TaurosFrontpic:           INCBIN "gfx/pokemon/tauros/front.2bpp.lz"
TaurosBackpic:            INCBIN "gfx/pokemon/tauros/back.2bpp.lz"
LaprasFrontpic:           INCBIN "gfx/pokemon/lapras/front.2bpp.lz"
LaprasBackpic:            INCBIN "gfx/pokemon/lapras/back.2bpp.lz"
DittoFrontpic:            INCBIN "gfx/pokemon/ditto/front.2bpp.lz"
DittoBackpic:             INCBIN "gfx/pokemon/ditto/back.2bpp.lz"
EeveeFrontpic:            INCBIN "gfx/pokemon/eevee/front.2bpp.lz"
EeveeBackpic:             INCBIN "gfx/pokemon/eevee/back.2bpp.lz"
VaporeonFrontpic:         INCBIN "gfx/pokemon/vaporeon/front.2bpp.lz"
VaporeonBackpic:          INCBIN "gfx/pokemon/vaporeon/back.2bpp.lz"


SECTION "Pokemon Pics 12", ROMX


JolteonFrontpic:          INCBIN "gfx/pokemon/jolteon/front.2bpp.lz"
JolteonBackpic:           INCBIN "gfx/pokemon/jolteon/back.2bpp.lz"
FlareonFrontpic:          INCBIN "gfx/pokemon/flareon/front.2bpp.lz"
FlareonBackpic:           INCBIN "gfx/pokemon/flareon/back.2bpp.lz"
PorygonFrontpic:          INCBIN "gfx/pokemon/porygon/front.2bpp.lz"
PorygonBackpic:           INCBIN "gfx/pokemon/porygon/back.2bpp.lz"
AerodactylFrontpic:       INCBIN "gfx/pokemon/aerodactyl/front.2bpp.lz"
AerodactylBackpic:        INCBIN "gfx/pokemon/aerodactyl/back.2bpp.lz"
SnorlaxFrontpic:          INCBIN "gfx/pokemon/snorlax/front.2bpp.lz"
SnorlaxBackpic:           INCBIN "gfx/pokemon/snorlax/back.2bpp.lz"
DratiniFrontpic:          INCBIN "gfx/pokemon/dratini/front.2bpp.lz"
DratiniBackpic:           INCBIN "gfx/pokemon/dratini/back.2bpp.lz"
DragonairFrontpic:        INCBIN "gfx/pokemon/dragonair/front.2bpp.lz"
DragonairBackpic:         INCBIN "gfx/pokemon/dragonair/back.2bpp.lz"
DragoniteFrontpic:        INCBIN "gfx/pokemon/dragonite/front.2bpp.lz"
DragoniteBackpic:         INCBIN "gfx/pokemon/dragonite/back.2bpp.lz"


SECTION "Pokemon Pics 13", ROMX


ChikoritaFrontpic:        INCBIN "gfx/pokemon/chikorita/front.2bpp.lz"
ChikoritaBackpic:         INCBIN "gfx/pokemon/chikorita/back.2bpp.lz"
BayleefFrontpic:          INCBIN "gfx/pokemon/bayleef/front.2bpp.lz"
BayleefBackpic:           INCBIN "gfx/pokemon/bayleef/back.2bpp.lz"
MeganiumFrontpic:         INCBIN "gfx/pokemon/meganium/front.2bpp.lz"
MeganiumBackpic:          INCBIN "gfx/pokemon/meganium/back.2bpp.lz"
CyndaquilFrontpic:        INCBIN "gfx/pokemon/cyndaquil/front.2bpp.lz"
CyndaquilBackpic:         INCBIN "gfx/pokemon/cyndaquil/back.2bpp.lz"
QuilavaFrontpic:          INCBIN "gfx/pokemon/quilava/front.2bpp.lz"
QuilavaBackpic:           INCBIN "gfx/pokemon/quilava/back.2bpp.lz"
TyphlosionFrontpic:       INCBIN "gfx/pokemon/typhlosion/front.2bpp.lz"
TyphlosionBackpic:        INCBIN "gfx/pokemon/typhlosion/back.2bpp.lz"
TotodileFrontpic:         INCBIN "gfx/pokemon/totodile/front.2bpp.lz"
TotodileBackpic:          INCBIN "gfx/pokemon/totodile/back.2bpp.lz"
CroconawFrontpic:         INCBIN "gfx/pokemon/croconaw/front.2bpp.lz"
CroconawBackpic:          INCBIN "gfx/pokemon/croconaw/back.2bpp.lz"


SECTION "Pokemon Pics 14", ROMX


FeraligatrFrontpic:       INCBIN "gfx/pokemon/feraligatr/front.2bpp.lz"
FeraligatrBackpic:        INCBIN "gfx/pokemon/feraligatr/back.2bpp.lz"
LedybaFrontpic:           INCBIN "gfx/pokemon/ledyba/front.2bpp.lz"
LedybaBackpic:            INCBIN "gfx/pokemon/ledyba/back.2bpp.lz"
LedianFrontpic:           INCBIN "gfx/pokemon/ledian/front.2bpp.lz"
LedianBackpic:            INCBIN "gfx/pokemon/ledian/back.2bpp.lz"
CrobatFrontpic:           INCBIN "gfx/pokemon/crobat/front.2bpp.lz"
CrobatBackpic:            INCBIN "gfx/pokemon/crobat/back.2bpp.lz"
ChinchouFrontpic:         INCBIN "gfx/pokemon/chinchou/front.2bpp.lz"
ChinchouBackpic:          INCBIN "gfx/pokemon/chinchou/back.2bpp.lz"
LanturnFrontpic:          INCBIN "gfx/pokemon/lanturn/front.2bpp.lz"
LanturnBackpic:           INCBIN "gfx/pokemon/lanturn/back.2bpp.lz"
TogepiFrontpic:           INCBIN "gfx/pokemon/togepi/front.2bpp.lz"
TogepiBackpic:            INCBIN "gfx/pokemon/togepi/back.2bpp.lz"
TogeticFrontpic:          INCBIN "gfx/pokemon/togetic/front.2bpp.lz"
TogeticBackpic:           INCBIN "gfx/pokemon/togetic/back.2bpp.lz"


SECTION "Pokemon Pics 15", ROMX


NatuFrontpic:             INCBIN "gfx/pokemon/natu/front.2bpp.lz"
NatuBackpic:              INCBIN "gfx/pokemon/natu/back.2bpp.lz"
XatuFrontpic:             INCBIN "gfx/pokemon/xatu/front.2bpp.lz"
XatuBackpic:              INCBIN "gfx/pokemon/xatu/back.2bpp.lz"
MareepFrontpic:           INCBIN "gfx/pokemon/mareep/front.2bpp.lz"
MareepBackpic:            INCBIN "gfx/pokemon/mareep/back.2bpp.lz"
FlaaffyFrontpic:          INCBIN "gfx/pokemon/flaaffy/front.2bpp.lz"
FlaaffyBackpic:           INCBIN "gfx/pokemon/flaaffy/back.2bpp.lz"
AmpharosFrontpic:         INCBIN "gfx/pokemon/ampharos/front.2bpp.lz"
AmpharosBackpic:          INCBIN "gfx/pokemon/ampharos/back.2bpp.lz"
BellossomFrontpic:        INCBIN "gfx/pokemon/bellossom/front.2bpp.lz"
BellossomBackpic:         INCBIN "gfx/pokemon/bellossom/back.2bpp.lz"
SudowoodoFrontpic:        INCBIN "gfx/pokemon/sudowoodo/front.2bpp.lz"
SudowoodoBackpic:         INCBIN "gfx/pokemon/sudowoodo/back.2bpp.lz"
PolitoedFrontpic:         INCBIN "gfx/pokemon/politoed/front.2bpp.lz"
PolitoedBackpic:          INCBIN "gfx/pokemon/politoed/back.2bpp.lz"


SECTION "Pokemon Pics 16", ROMX


WooperFrontpic:           INCBIN "gfx/pokemon/wooper/front.2bpp.lz"
WooperBackpic:            INCBIN "gfx/pokemon/wooper/back.2bpp.lz"
QuagsireFrontpic:         INCBIN "gfx/pokemon/quagsire/front.2bpp.lz"
QuagsireBackpic:          INCBIN "gfx/pokemon/quagsire/back.2bpp.lz"
EspeonFrontpic:           INCBIN "gfx/pokemon/espeon/front.2bpp.lz"
EspeonBackpic:            INCBIN "gfx/pokemon/espeon/back.2bpp.lz"
UmbreonFrontpic:          INCBIN "gfx/pokemon/umbreon/front.2bpp.lz"
UmbreonBackpic:           INCBIN "gfx/pokemon/umbreon/back.2bpp.lz"
MurkrowFrontpic:          INCBIN "gfx/pokemon/murkrow/front.2bpp.lz"
MurkrowBackpic:           INCBIN "gfx/pokemon/murkrow/back.2bpp.lz"
SlowkingFrontpic:         INCBIN "gfx/pokemon/slowking/front.2bpp.lz"
SlowkingBackpic:          INCBIN "gfx/pokemon/slowking/back.2bpp.lz"
GirafarigFrontpic:        INCBIN "gfx/pokemon/girafarig/front.2bpp.lz"
GirafarigBackpic:         INCBIN "gfx/pokemon/girafarig/back.2bpp.lz"
PinecoFrontpic:           INCBIN "gfx/pokemon/pineco/front.2bpp.lz"
PinecoBackpic:            INCBIN "gfx/pokemon/pineco/back.2bpp.lz"
ForretressFrontpic:       INCBIN "gfx/pokemon/forretress/front.2bpp.lz"
ForretressBackpic:        INCBIN "gfx/pokemon/forretress/back.2bpp.lz"


SECTION "Pokemon Pics 17", ROMX


GligarFrontpic:           INCBIN "gfx/pokemon/gligar/front.2bpp.lz"
GligarBackpic:            INCBIN "gfx/pokemon/gligar/back.2bpp.lz"
SteelixFrontpic:          INCBIN "gfx/pokemon/steelix/front.2bpp.lz"
SteelixBackpic:           INCBIN "gfx/pokemon/steelix/back.2bpp.lz"
SnubbullFrontpic:         INCBIN "gfx/pokemon/snubbull/front.2bpp.lz"
SnubbullBackpic:          INCBIN "gfx/pokemon/snubbull/back.2bpp.lz"
GranbullFrontpic:         INCBIN "gfx/pokemon/granbull/front.2bpp.lz"
GranbullBackpic:          INCBIN "gfx/pokemon/granbull/back.2bpp.lz"
ScizorFrontpic:           INCBIN "gfx/pokemon/scizor/front.2bpp.lz"
ScizorBackpic:            INCBIN "gfx/pokemon/scizor/back.2bpp.lz"
HeracrossFrontpic:        INCBIN "gfx/pokemon/heracross/front.2bpp.lz"
HeracrossBackpic:         INCBIN "gfx/pokemon/heracross/back.2bpp.lz"
SneaselFrontpic:          INCBIN "gfx/pokemon/sneasel/front.2bpp.lz"
SneaselBackpic:           INCBIN "gfx/pokemon/sneasel/back.2bpp.lz"
TeddiursaFrontpic:        INCBIN "gfx/pokemon/teddiursa/front.2bpp.lz"
TeddiursaBackpic:         INCBIN "gfx/pokemon/teddiursa/back.2bpp.lz"


SECTION "Pokemon Pics 18", ROMX


UrsaringFrontpic:         INCBIN "gfx/pokemon/ursaring/front.2bpp.lz"
UrsaringBackpic:          INCBIN "gfx/pokemon/ursaring/back.2bpp.lz"
SlugmaFrontpic:           INCBIN "gfx/pokemon/slugma/front.2bpp.lz"
SlugmaBackpic:            INCBIN "gfx/pokemon/slugma/back.2bpp.lz"
MagcargoFrontpic:         INCBIN "gfx/pokemon/magcargo/front.2bpp.lz"
MagcargoBackpic:          INCBIN "gfx/pokemon/magcargo/back.2bpp.lz"
SwinubFrontpic:           INCBIN "gfx/pokemon/swinub/front.2bpp.lz"
SwinubBackpic:            INCBIN "gfx/pokemon/swinub/back.2bpp.lz"
PiloswineFrontpic:        INCBIN "gfx/pokemon/piloswine/front.2bpp.lz"
PiloswineBackpic:         INCBIN "gfx/pokemon/piloswine/back.2bpp.lz"
CorsolaFrontpic:          INCBIN "gfx/pokemon/corsola/front.2bpp.lz"
CorsolaBackpic:           INCBIN "gfx/pokemon/corsola/back.2bpp.lz"
SkarmoryFrontpic:         INCBIN "gfx/pokemon/skarmory/front.2bpp.lz"
SkarmoryBackpic:          INCBIN "gfx/pokemon/skarmory/back.2bpp.lz"
HoundourFrontpic:         INCBIN "gfx/pokemon/houndour/front.2bpp.lz"
HoundourBackpic:          INCBIN "gfx/pokemon/houndour/back.2bpp.lz"


SECTION "Pokemon Pics 19", ROMX


HoundoomFrontpic:         INCBIN "gfx/pokemon/houndoom/front.2bpp.lz"
HoundoomBackpic:          INCBIN "gfx/pokemon/houndoom/back.2bpp.lz"
DragapultFrontpic:        INCBIN "gfx/pokemon/dragapult/front.2bpp.lz"
DragapultBackpic:         INCBIN "gfx/pokemon/dragapult/back.2bpp.lz"
PhanpyFrontpic:           INCBIN "gfx/pokemon/phanpy/front.2bpp.lz"
PhanpyBackpic:            INCBIN "gfx/pokemon/phanpy/back.2bpp.lz"
DonphanFrontpic:          INCBIN "gfx/pokemon/donphan/front.2bpp.lz"
DonphanBackpic:           INCBIN "gfx/pokemon/donphan/back.2bpp.lz"
Porygon2Frontpic:         INCBIN "gfx/pokemon/porygon2/front.2bpp.lz"
Porygon2Backpic:          INCBIN "gfx/pokemon/porygon2/back.2bpp.lz"
StantlerFrontpic:         INCBIN "gfx/pokemon/stantler/front.2bpp.lz"
StantlerBackpic:          INCBIN "gfx/pokemon/stantler/back.2bpp.lz"
SmeargleFrontpic:         INCBIN "gfx/pokemon/smeargle/front.2bpp.lz"
SmeargleBackpic:          INCBIN "gfx/pokemon/smeargle/back.2bpp.lz"
ElekidFrontpic:           INCBIN "gfx/pokemon/elekid/front.2bpp.lz"
ElekidBackpic:            INCBIN "gfx/pokemon/elekid/back.2bpp.lz"
MagbyFrontpic:            INCBIN "gfx/pokemon/magby/front.2bpp.lz"
MagbyBackpic:             INCBIN "gfx/pokemon/magby/back.2bpp.lz"


SECTION "Pokemon Pics 20", ROMX


MiltankFrontpic:          INCBIN "gfx/pokemon/miltank/front.2bpp.lz"
MiltankBackpic:           INCBIN "gfx/pokemon/miltank/back.2bpp.lz"
BlisseyFrontpic:          INCBIN "gfx/pokemon/blissey/front.2bpp.lz"
BlisseyBackpic:           INCBIN "gfx/pokemon/blissey/back.2bpp.lz"
LarvitarFrontpic:         INCBIN "gfx/pokemon/larvitar/front.2bpp.lz"
LarvitarBackpic:          INCBIN "gfx/pokemon/larvitar/back.2bpp.lz"
PupitarFrontpic:          INCBIN "gfx/pokemon/pupitar/front.2bpp.lz"
PupitarBackpic:           INCBIN "gfx/pokemon/pupitar/back.2bpp.lz"
TyranitarFrontpic:        INCBIN "gfx/pokemon/tyranitar/front.2bpp.lz"
TyranitarBackpic:         INCBIN "gfx/pokemon/tyranitar/back.2bpp.lz"
PikachuFrontpic:          INCBIN "gfx/pokemon/pikachu/front.2bpp.lz"
PikachuBackpic:           INCBIN "gfx/pokemon/pikachu/back.2bpp.lz"
MagikarpFrontpic:         INCBIN "gfx/pokemon/magikarp/front.2bpp.lz"
MagikarpBackpic:          INCBIN "gfx/pokemon/magikarp/back.2bpp.lz"
GyaradosFrontpic:         INCBIN "gfx/pokemon/gyarados/front.2bpp.lz"
GyaradosBackpic:          INCBIN "gfx/pokemon/gyarados/back.2bpp.lz"
Gyarados_RGFrontpic:      INCBIN "gfx/pokemon/gyarados_rg/front.2bpp.lz"


SECTION "Pokemon Pics 21", ROMX

WingullFrontpic:          INCBIN "gfx/pokemon/wingull/front.2bpp.lz"
WingullBackpic:           INCBIN "gfx/pokemon/wingull/back.2bpp.lz"
PelipperFrontpic:         INCBIN "gfx/pokemon/pelipper/front.2bpp.lz"
PelipperBackpic:          INCBIN "gfx/pokemon/pelipper/back.2bpp.lz"
RaltsFrontpic:            INCBIN "gfx/pokemon/ralts/front.2bpp.lz"
RaltsBackpic:             INCBIN "gfx/pokemon/ralts/back.2bpp.lz"
KirliaFrontpic:           INCBIN "gfx/pokemon/kirlia/front.2bpp.lz"
KirliaBackpic:            INCBIN "gfx/pokemon/kirlia/back.2bpp.lz"
GardevoirFrontpic:        INCBIN "gfx/pokemon/gardevoir/front.2bpp.lz"
GardevoirBackpic:         INCBIN "gfx/pokemon/gardevoir/back.2bpp.lz"
ShroomishFrontpic:        INCBIN "gfx/pokemon/shroomish/front.2bpp.lz"
ShroomishBackpic:         INCBIN "gfx/pokemon/shroomish/back.2bpp.lz"
BreloomFrontpic:          INCBIN "gfx/pokemon/breloom/front.2bpp.lz"
BreloomBackpic:           INCBIN "gfx/pokemon/breloom/back.2bpp.lz"
AronFrontpic:      		  INCBIN "gfx/pokemon/aron/front.2bpp.lz"
AronBackpic:        	  INCBIN "gfx/pokemon/aron/back.2bpp.lz"
LaironFrontpic:           INCBIN "gfx/pokemon/lairon/front.2bpp.lz"
LaironBackpic:            INCBIN "gfx/pokemon/lairon/back.2bpp.lz"
AggronFrontpic:           INCBIN "gfx/pokemon/aggron/front.2bpp.lz"
AggronBackpic:            INCBIN "gfx/pokemon/aggron/back.2bpp.lz"
MedititeFrontpic:         INCBIN "gfx/pokemon/meditite/front.2bpp.lz"
MedititeBackpic:          INCBIN "gfx/pokemon/meditite/back.2bpp.lz"
MedichamFrontpic:         INCBIN "gfx/pokemon/medicham/front.2bpp.lz"
MedichamBackpic:          INCBIN "gfx/pokemon/medicham/back.2bpp.lz"
CarvanhaFrontpic:         INCBIN "gfx/pokemon/carvanha/front.2bpp.lz"
CarvanhaBackpic:          INCBIN "gfx/pokemon/carvanha/back.2bpp.lz"
SharpedoFrontpic:         INCBIN "gfx/pokemon/sharpedo/front.2bpp.lz"
SharpedoBackpic:          INCBIN "gfx/pokemon/sharpedo/back.2bpp.lz"


SECTION "Pokemon Pics 22", ROMX


NumelFrontpic:            INCBIN "gfx/pokemon/numel/front.2bpp.lz"
NumelBackpic:             INCBIN "gfx/pokemon/numel/back.2bpp.lz"
CameruptFrontpic:         INCBIN "gfx/pokemon/camerupt/front.2bpp.lz"
CameruptBackpic:          INCBIN "gfx/pokemon/camerupt/back.2bpp.lz"
SwabluFrontpic:           INCBIN "gfx/pokemon/swablu/front.2bpp.lz"
SwabluBackpic:            INCBIN "gfx/pokemon/swablu/back.2bpp.lz"
AltariaFrontpic:          INCBIN "gfx/pokemon/altaria/front.2bpp.lz"
AltariaBackpic:           INCBIN "gfx/pokemon/altaria/back.2bpp.lz"
SnoruntFrontpic:          INCBIN "gfx/pokemon/snorunt/front.2bpp.lz"
SnoruntBackpic:           INCBIN "gfx/pokemon/snorunt/back.2bpp.lz"
GlalieFrontpic:           INCBIN "gfx/pokemon/glalie/front.2bpp.lz"
GlalieBackpic:            INCBIN "gfx/pokemon/glalie/back.2bpp.lz"
BuizelFrontpic:           INCBIN "gfx/pokemon/buizel/front.2bpp.lz"
BuizelBackpic:            INCBIN "gfx/pokemon/buizel/back.2bpp.lz"
FloatzelFrontpic:         INCBIN "gfx/pokemon/floatzel/front.2bpp.lz"
FloatzelBackpic:          INCBIN "gfx/pokemon/floatzel/back.2bpp.lz"
DrifloonFrontpic:         INCBIN "gfx/pokemon/drifloon/front.2bpp.lz"
DrifloonBackpic:          INCBIN "gfx/pokemon/drifloon/back.2bpp.lz"
DrifblimFrontpic:         INCBIN "gfx/pokemon/drifblim/front.2bpp.lz"
DrifblimBackpic:          INCBIN "gfx/pokemon/drifblim/back.2bpp.lz"
BunearyFrontpic:          INCBIN "gfx/pokemon/buneary/front.2bpp.lz"
BunearyBackpic:           INCBIN "gfx/pokemon/buneary/back.2bpp.lz"


SECTION "Pokemon Pics 23", ROMX


LopunnyFrontpic:          INCBIN "gfx/pokemon/lopunny/front.2bpp.lz"
LopunnyBackpic:           INCBIN "gfx/pokemon/lopunny/back.2bpp.lz"
HonchkrowFrontpic:        INCBIN "gfx/pokemon/honchkrow/front.2bpp.lz"
HonchkrowBackpic:         INCBIN "gfx/pokemon/honchkrow/back.2bpp.lz"
SpiritombFrontpic:        INCBIN "gfx/pokemon/spiritomb/front.2bpp.lz"
SpiritombBackpic:         INCBIN "gfx/pokemon/spiritomb/back.2bpp.lz"
MunchlaxFrontpic:         INCBIN "gfx/pokemon/munchlax/front.2bpp.lz"
MunchlaxBackpic:          INCBIN "gfx/pokemon/munchlax/back.2bpp.lz"
RioluFrontpic:        	  INCBIN "gfx/pokemon/riolu/front.2bpp.lz"
RioluBackpic:          	  INCBIN "gfx/pokemon/riolu/back.2bpp.lz"
LucarioFrontpic:          INCBIN "gfx/pokemon/lucario/front.2bpp.lz"
LucarioBackpic:           INCBIN "gfx/pokemon/lucario/back.2bpp.lz"
CroagunkFrontpic:         INCBIN "gfx/pokemon/croagunk/front.2bpp.lz"
CroagunkBackpic:          INCBIN "gfx/pokemon/croagunk/back.2bpp.lz"
ToxicroakFrontpic:        INCBIN "gfx/pokemon/toxicroak/front.2bpp.lz"
ToxicroakBackpic:         INCBIN "gfx/pokemon/toxicroak/back.2bpp.lz"
SnoverFrontpic:           INCBIN "gfx/pokemon/snover/front.2bpp.lz"
SnoverBackpic:            INCBIN "gfx/pokemon/snover/back.2bpp.lz"
AbomasnowFrontpic:        INCBIN "gfx/pokemon/abomasnow/front.2bpp.lz"
AbomasnowBackpic:         INCBIN "gfx/pokemon/abomasnow/back.2bpp.lz"
WeavileFrontpic:          INCBIN "gfx/pokemon/weavile/front.2bpp.lz"
WeavileBackpic:           INCBIN "gfx/pokemon/weavile/back.2bpp.lz"
MagnezoneFrontpic:        INCBIN "gfx/pokemon/magnezone/front.2bpp.lz"
MagnezoneBackpic:         INCBIN "gfx/pokemon/magnezone/back.2bpp.lz"
ElectivireFrontpic:       INCBIN "gfx/pokemon/electivire/front.2bpp.lz"
ElectivireBackpic:        INCBIN "gfx/pokemon/electivire/back.2bpp.lz"
MagmortarFrontpic:        INCBIN "gfx/pokemon/magmortar/front.2bpp.lz"
MagmortarBackpic:         INCBIN "gfx/pokemon/magmortar/back.2bpp.lz"


SECTION "Pokemon Pics 24", ROMX


TogekissFrontpic:         INCBIN "gfx/pokemon/togekiss/front.2bpp.lz"
TogekissBackpic:          INCBIN "gfx/pokemon/togekiss/back.2bpp.lz"
LeafeonFrontpic:          INCBIN "gfx/pokemon/leafeon/front.2bpp.lz"
LeafeonBackpic:           INCBIN "gfx/pokemon/leafeon/back.2bpp.lz"
GlaceonFrontpic:          INCBIN "gfx/pokemon/glaceon/front.2bpp.lz"
GlaceonBackpic:           INCBIN "gfx/pokemon/glaceon/back.2bpp.lz"
GliscorFrontpic:          INCBIN "gfx/pokemon/gliscor/front.2bpp.lz"
GliscorBackpic:           INCBIN "gfx/pokemon/gliscor/back.2bpp.lz"
MamoswineFrontpic:        INCBIN "gfx/pokemon/mamoswine/front.2bpp.lz"
MamoswineBackpic:         INCBIN "gfx/pokemon/mamoswine/back.2bpp.lz"
PorygonZFrontpic:         INCBIN "gfx/pokemon/porygon_z/front.2bpp.lz"
PorygonZBackpic:          INCBIN "gfx/pokemon/porygon_z/back.2bpp.lz"
GalladeFrontpic:       	  INCBIN "gfx/pokemon/gallade/front.2bpp.lz"
GalladeBackpic:       	  INCBIN "gfx/pokemon/gallade/back.2bpp.lz"
FroslassFrontpic:         INCBIN "gfx/pokemon/froslass/front.2bpp.lz"
FroslassBackpic:       	  INCBIN "gfx/pokemon/froslass/back.2bpp.lz"
DrilburFrontpic:          INCBIN "gfx/pokemon/drilbur/front.2bpp.lz"
DrilburBackpic:           INCBIN "gfx/pokemon/drilbur/back.2bpp.lz"
ExcadrillFrontpic:        INCBIN "gfx/pokemon/excadrill/front.2bpp.lz"
ExcadrillBackpic:         INCBIN "gfx/pokemon/excadrill/back.2bpp.lz"
VenipedeFrontpic:         INCBIN "gfx/pokemon/venipede/front.2bpp.lz"
VenipedeBackpic:          INCBIN "gfx/pokemon/venipede/back.2bpp.lz"
WhirlipedeFrontpic:       INCBIN "gfx/pokemon/whirlipede/front.2bpp.lz"
WhirlipedeBackpic:        INCBIN "gfx/pokemon/whirlipede/back.2bpp.lz"
ScolipedeFrontpic:        INCBIN "gfx/pokemon/scolipede/front.2bpp.lz"
ScolipedeBackpic:         INCBIN "gfx/pokemon/scolipede/back.2bpp.lz"
CottoneeFrontpic:         INCBIN "gfx/pokemon/cottonee/front.2bpp.lz"
CottoneeBackpic:          INCBIN "gfx/pokemon/cottonee/back.2bpp.lz"
WhimsicottFrontpic:       INCBIN "gfx/pokemon/whimsicott/front.2bpp.lz"
WhimsicottBackpic:        INCBIN "gfx/pokemon/whimsicott/back.2bpp.lz"


SECTION "Pokemon Pics 25", ROMX

SandileFrontpic:          INCBIN "gfx/pokemon/sandile/front.2bpp.lz"
SandileBackpic:           INCBIN "gfx/pokemon/sandile/back.2bpp.lz"
KrokorokFrontpic:         INCBIN "gfx/pokemon/krokorok/front.2bpp.lz"
KrokorokBackpic:          INCBIN "gfx/pokemon/krokorok/back.2bpp.lz"
KrookodileFrontpic:       INCBIN "gfx/pokemon/krookodile/front.2bpp.lz"
KrookodileBackpic:        INCBIN "gfx/pokemon/krookodile/back.2bpp.lz"
MaractusFrontpic:         INCBIN "gfx/pokemon/maractus/front.2bpp.lz"
MaractusBackpic:          INCBIN "gfx/pokemon/maractus/back.2bpp.lz"
DwebbleFrontpic:          INCBIN "gfx/pokemon/dwebble/front.2bpp.lz"
DwebbleBackpic:           INCBIN "gfx/pokemon/dwebble/back.2bpp.lz"
CrustleFrontpic:          INCBIN "gfx/pokemon/crustle/front.2bpp.lz"
CrustleBackpic:           INCBIN "gfx/pokemon/crustle/back.2bpp.lz"
ScraggyFrontpic:          INCBIN "gfx/pokemon/scraggy/front.2bpp.lz"
ScraggyBackpic:           INCBIN "gfx/pokemon/scraggy/back.2bpp.lz"
ScraftyFrontpic:          INCBIN "gfx/pokemon/scrafty/front.2bpp.lz"
ScraftyBackpic:           INCBIN "gfx/pokemon/scrafty/back.2bpp.lz"
TirtougaFrontpic:         INCBIN "gfx/pokemon/tirtouga/front.2bpp.lz"
TirtougaBackpic:          INCBIN "gfx/pokemon/tirtouga/back.2bpp.lz"
CarracostaFrontpic:       INCBIN "gfx/pokemon/carracosta/front.2bpp.lz"
CarracostaBackpic:        INCBIN "gfx/pokemon/carracosta/back.2bpp.lz"
ArchenFrontpic:           INCBIN "gfx/pokemon/archen/front.2bpp.lz"
ArchenBackpic:            INCBIN "gfx/pokemon/archen/back.2bpp.lz"
ArcheopsFrontpic:         INCBIN "gfx/pokemon/archeops/front.2bpp.lz"
ArcheopsBackpic:          INCBIN "gfx/pokemon/archeops/back.2bpp.lz"
MinccinoFrontpic:         INCBIN "gfx/pokemon/minccino/front.2bpp.lz"
MinccinoBackpic:          INCBIN "gfx/pokemon/minccino/back.2bpp.lz"
CinccinoFrontpic:         INCBIN "gfx/pokemon/cinccino/front.2bpp.lz"
CinccinoBackpic:          INCBIN "gfx/pokemon/cinccino/back.2bpp.lz"
JoltikFrontpic:           INCBIN "gfx/pokemon/joltik/front.2bpp.lz"
JoltikBackpic:            INCBIN "gfx/pokemon/joltik/back.2bpp.lz"
GalvantulaFrontpic:       INCBIN "gfx/pokemon/galvantula/front.2bpp.lz"
GalvantulaBackpic:        INCBIN "gfx/pokemon/galvantula/back.2bpp.lz"
LitwickFrontpic:          INCBIN "gfx/pokemon/litwick/front.2bpp.lz"
LitwickBackpic:           INCBIN "gfx/pokemon/litwick/back.2bpp.lz"


SECTION "Pokemon Pics 26", ROMX

LampentFrontpic:          INCBIN "gfx/pokemon/lampent/front.2bpp.lz"
LampentBackpic:           INCBIN "gfx/pokemon/lampent/back.2bpp.lz"
ChandelureFrontpic:       INCBIN "gfx/pokemon/chandelure/front.2bpp.lz"
ChandelureBackpic:        INCBIN "gfx/pokemon/chandelure/back.2bpp.lz"
LarvestaFrontpic:         INCBIN "gfx/pokemon/larvesta/front.2bpp.lz"
LarvestaBackpic:          INCBIN "gfx/pokemon/larvesta/back.2bpp.lz"
VolcaronaFrontpic:        INCBIN "gfx/pokemon/volcarona/front.2bpp.lz"
VolcaronaBackpic:         INCBIN "gfx/pokemon/volcarona/back.2bpp.lz"
SylveonFrontpic:          INCBIN "gfx/pokemon/sylveon/front.2bpp.lz"
SylveonBackpic:           INCBIN "gfx/pokemon/sylveon/back.2bpp.lz"
PhantumpFrontpic:         INCBIN "gfx/pokemon/phantump/front.2bpp.lz"
PhantumpBackpic:          INCBIN "gfx/pokemon/phantump/back.2bpp.lz"
TrevenantFrontpic:        INCBIN "gfx/pokemon/trevenant/front.2bpp.lz"
TrevenantBackpic:         INCBIN "gfx/pokemon/trevenant/back.2bpp.lz"
NoibatFrontpic:           INCBIN "gfx/pokemon/noibat/front.2bpp.lz"
NoibatBackpic:            INCBIN "gfx/pokemon/noibat/back.2bpp.lz"
NoivernFrontpic:          INCBIN "gfx/pokemon/noivern/front.2bpp.lz"
NoivernBackpic:           INCBIN "gfx/pokemon/noivern/back.2bpp.lz"
PikipekFrontpic:          INCBIN "gfx/pokemon/pikipek/front.2bpp.lz"
PikipekBackpic:           INCBIN "gfx/pokemon/pikipek/back.2bpp.lz"
TrumbeakFrontpic:         INCBIN "gfx/pokemon/trumbeak/front.2bpp.lz"
TrumbeakBackpic:          INCBIN "gfx/pokemon/trumbeak/back.2bpp.lz"
ToucannonFrontpic:        INCBIN "gfx/pokemon/toucannon/front.2bpp.lz"
ToucannonBackpic:         INCBIN "gfx/pokemon/toucannon/back.2bpp.lz"
MareanieFrontpic:         INCBIN "gfx/pokemon/mareanie/front.2bpp.lz"
MareanieBackpic:          INCBIN "gfx/pokemon/mareanie/back.2bpp.lz"
ToxapexFrontpic:          INCBIN "gfx/pokemon/toxapex/front.2bpp.lz"
ToxapexBackpic:           INCBIN "gfx/pokemon/toxapex/back.2bpp.lz"

SECTION "Pokemon Pics 27", ROMX

WyrdeerFrontpic:     	  INCBIN "gfx/pokemon/wyrdeer/front.2bpp.lz"
WyrdeerBackpic:           INCBIN "gfx/pokemon/wyrdeer/back.2bpp.lz"
MarillFrontpic:      	  INCBIN "gfx/pokemon/marill/front.2bpp.lz"
MarillBackpic:       	  INCBIN "gfx/pokemon/marill/back.2bpp.lz"
AzumarillFrontpic:        INCBIN "gfx/pokemon/azumarill/front.2bpp.lz"
AzumarillBackpic:         INCBIN "gfx/pokemon/azumarill/back.2bpp.lz"
Raichu_AFrontpic:         INCBIN "gfx/pokemon/raichu_a/front.2bpp.lz"
Raichu_ABackpic:          INCBIN "gfx/pokemon/raichu_a/back.2bpp.lz"
Marowak_AFrontpic:        INCBIN "gfx/pokemon/marowak_a/front.2bpp.lz"
Marowak_ABackpic:         INCBIN "gfx/pokemon/marowak_a/back.2bpp.lz"
Grimer_AFrontpic:         INCBIN "gfx/pokemon/grimer_a/front.2bpp.lz"
Grimer_ABackpic:          INCBIN "gfx/pokemon/grimer_a/back.2bpp.lz"
Muk_AFrontpic:        	  INCBIN "gfx/pokemon/muk_a/front.2bpp.lz"
Muk_ABackpic:         	  INCBIN "gfx/pokemon/muk_a/back.2bpp.lz"
Sandshrew_AFrontpic:      INCBIN "gfx/pokemon/sandshrew_a/front.2bpp.lz"
Sandshrew_ABackpic:       INCBIN "gfx/pokemon/sandshrew_a/back.2bpp.lz"
Sandslash_AFrontpic:      INCBIN "gfx/pokemon/sandslash_a/front.2bpp.lz"
Sandslash_ABackpic:       INCBIN "gfx/pokemon/sandslash_a/back.2bpp.lz"
Corsola_GFrontpic:        INCBIN "gfx/pokemon/corsola_g/front.2bpp.lz"
Corsola_GBackpic:         INCBIN "gfx/pokemon/corsola_g/back.2bpp.lz"
CursolaFrontpic:          INCBIN "gfx/pokemon/cursola/front.2bpp.lz"
CursolaBackpic:           INCBIN "gfx/pokemon/cursola/back.2bpp.lz"
FarigirafFrontpic:        INCBIN "gfx/pokemon/farigiraf/front.2bpp.lz"
FarigirafBackpic:         INCBIN "gfx/pokemon/farigiraf/back.2bpp.lz"
Meowth_AFrontpic:         INCBIN "gfx/pokemon/meowth_a/front.2bpp.lz"
Meowth_ABackpic:          INCBIN "gfx/pokemon/meowth_a/back.2bpp.lz"
Persian_AFrontpic:        INCBIN "gfx/pokemon/persian_a/front.2bpp.lz"
Persian_ABackpic:         INCBIN "gfx/pokemon/persian_a/back.2bpp.lz"

SECTION "Pokemon Pics 28", ROMX

MimikyuFrontpic:          INCBIN "gfx/pokemon/mimikyu/front.2bpp.lz"
MimikyuBackpic:           INCBIN "gfx/pokemon/mimikyu/back.2bpp.lz"
Mimikyu_BFrontpic:        INCBIN "gfx/pokemon/mimikyu_b/front.2bpp.lz"
Mimikyu_BBackpic:         INCBIN "gfx/pokemon/mimikyu_b/back.2bpp.lz"
CherubiFrontpic:          INCBIN "gfx/pokemon/cherubi/front.2bpp.lz"
CherubiBackpic:           INCBIN "gfx/pokemon/cherubi/back.2bpp.lz"
CherrimFrontpic:          INCBIN "gfx/pokemon/cherrim/front.2bpp.lz"
CherrimBackpic:           INCBIN "gfx/pokemon/cherrim/back.2bpp.lz"
Cherrim_SFrontpic:        INCBIN "gfx/pokemon/cherrim_s/front.2bpp.lz"
Cherrim_SBackpic:         INCBIN "gfx/pokemon/cherrim_s/back.2bpp.lz"
Wooper_PFrontpic:         INCBIN "gfx/pokemon/wooper_p/front.2bpp.lz"
Wooper_PBackpic:          INCBIN "gfx/pokemon/wooper_p/back.2bpp.lz"
ClodsireFrontpic:         INCBIN "gfx/pokemon/clodsire/front.2bpp.lz"
ClodsireBackpic:          INCBIN "gfx/pokemon/clodsire/back.2bpp.lz"
EggPic:                   INCBIN "gfx/pokemon/egg/front.2bpp.lz"


SECTION "Trainer Pics 1", ROMX

StanleyPic:               INCBIN "gfx/trainers/stanley.2bpp.lz"
RodneyPic:                INCBIN "gfx/trainers/rodney.2bpp.lz"
WendyPic:         	      INCBIN "gfx/trainers/wendy.2bpp.lz"
CharliePic:               INCBIN "gfx/trainers/charlie.2bpp.lz"
PollyPic:                 INCBIN "gfx/trainers/polly.2bpp.lz"
JasminePic:
PrycePic:
ClairPic:
WillPic:
KogaPic:
BrunoPic:
KarenPic:
ChampionPic:
RivalPic:                 INCBIN "gfx/trainers/rival.2bpp.lz"
YoungsterPic:             INCBIN "gfx/trainers/youngster.2bpp.lz"
BugCatcherPic:            INCBIN "gfx/trainers/bug_catcher.2bpp.lz"



SECTION "Trainer Pics 2", ROMX

CamperPic:                INCBIN "gfx/trainers/camper.2bpp.lz"
PicnickerPic:             INCBIN "gfx/trainers/picnicker.2bpp.lz"
TwinsPic:                 INCBIN "gfx/trainers/twins.2bpp.lz"
FisherPic:                INCBIN "gfx/trainers/fisher.2bpp.lz"
BirdKeeperPic:            INCBIN "gfx/trainers/bird_keeper.2bpp.lz"
HikerPic:                 INCBIN "gfx/trainers/hiker.2bpp.lz"
GruntmPic:                INCBIN "gfx/trainers/grunt_m.2bpp.lz"
GruntfPic:                INCBIN "gfx/trainers/grunt_f.2bpp.lz"
KagePic:				  INCBIN "gfx/trainers/kage.2bpp.lz"
PokefanmPic:              INCBIN "gfx/trainers/pokefan_m.2bpp.lz"
PokefanfPic:              INCBIN "gfx/trainers/pokefan_f.2bpp.lz"
OfficerPic:               INCBIN "gfx/trainers/officer.2bpp.lz"
PokemaniacPic:            INCBIN "gfx/trainers/pokemaniac.2bpp.lz"
SuperNerdPic:			  INCBIN "gfx/trainers/super_nerd.2bpp.lz"
LassPic:                  INCBIN "gfx/trainers/lass.2bpp.lz"
BeautyPic:                INCBIN "gfx/trainers/beauty.2bpp.lz"


SECTION "Trainer Pics 3", ROMX

FirebreatherPic:          INCBIN "gfx/trainers/firebreather.2bpp.lz"
JugglerPic:               INCBIN "gfx/trainers/juggler.2bpp.lz"
SchoolboyPic:             INCBIN "gfx/trainers/schoolboy.2bpp.lz"
PsychicTPic:              INCBIN "gfx/trainers/psychic_t.2bpp.lz"
CouplePic:                INCBIN "gfx/trainers/couple.2bpp.lz"
GentlemanPic:             INCBIN "gfx/trainers/gentleman.2bpp.lz"
LadyPic:                  INCBIN "gfx/trainers/lady.2bpp.lz"
SailorPic:                INCBIN "gfx/trainers/sailor.2bpp.lz"
SwimmermPic:              INCBIN "gfx/trainers/swimmer_m.2bpp.lz"
SwimmerfPic:              INCBIN "gfx/trainers/swimmer_f.2bpp.lz"
BurglarPic:               INCBIN "gfx/trainers/burglar.2bpp.lz"
ScientistPic:			  INCBIN "gfx/trainers/scientist.2bpp.lz"
BoarderPic:               INCBIN "gfx/trainers/boarder.2bpp.lz"


SECTION "Trainer Pics 4", ROMX

SkierPic:                 INCBIN "gfx/trainers/skier.2bpp.lz"
BlackbeltTPic:            INCBIN "gfx/trainers/blackbelt_t.2bpp.lz"
TeacherPic:               
TeacherFastPic:			  INCBIN "gfx/trainers/teacher.2bpp.lz"
GuitaristPic:          	  INCBIN "gfx/trainers/guitarist.2bpp.lz"
BikerPic:                 INCBIN "gfx/trainers/biker.2bpp.lz"
CooltrainermPic:          INCBIN "gfx/trainers/cooltrainer_m.2bpp.lz"
CooltrainerfPic:          INCBIN "gfx/trainers/cooltrainer_f.2bpp.lz"
ProfSprucePic:            INCBIN "gfx/trainers/spruce.2bpp.lz"
TrioBroRPic:
TrioBroBPic:
TrioBroYPic:			  INCBIN "gfx/trainers/triobro.2bpp.lz"
DisguiseManPic:			  INCBIN "gfx/trainers/disguiseman.2bpp.lz"
BikiniBRPic:			  INCBIN "gfx/trainers/bikinibr.2bpp.lz"
BikiniBLPic:			  INCBIN "gfx/trainers/bikinibl.2bpp.lz"
BikiniPPic:				  INCBIN "gfx/trainers/bikinip.2bpp.lz"


SECTION "Trainer Pics 5", ROMX

BunearymPic:;			  INCBIN "gfx/trainers/buneary_m.2bpp.lz"
DelinquentmPic:			  INCBIN "gfx/trainers/delinquent_m.2bpp.lz"
BunearyfPic:;			  INCBIN "gfx/trainers/buneary_f.2bpp.lz"
DelinquentfPic:			  INCBIN "gfx/trainers/delinquent_f.2bpp.lz"
FrankiePic:				  INCBIN "gfx/trainers/frankie.2bpp.lz"
SkaterPic:				  INCBIN "gfx/trainers/skater.2bpp.lz"
MallCopPic:				  INCBIN "gfx/trainers/mall_cop.2bpp.lz"
LockePic:				  INCBIN "gfx/trainers/locke.2bpp.lz"
SpaTrainerPic:			  INCBIN "gfx/trainers/spa_trainer.2bpp.lz"
SpaTrainerFPic:			  INCBIN "gfx/trainers/spa_trainer_f.2bpp.lz"
AromaLadyPic:			  INCBIN "gfx/trainers/aroma_lady.2bpp.lz"
TowerTycoonPic:
CoryPic:				  INCBIN "gfx/trainers/cory.2bpp.lz"
CoraPic:				  INCBIN "gfx/trainers/cora.2bpp.lz"
PippiPic:				  INCBIN "gfx/trainers/pippi.2bpp.lz"
RedPic:					  INCBIN "gfx/trainers/cory.2bpp.lz"
LeafPic:				  INCBIN "gfx/trainers/cora.2bpp.lz"
GoldPic:				  INCBIN "gfx/trainers/cory.2bpp.lz"
KrisPic:				  INCBIN "gfx/trainers/cora.2bpp.lz"


SECTION "Painting Pics", ROMX

HoOhPaintingPic:          INCBIN "gfx/paintings/ho_oh_painting.2bpp.lz"
LugiaPaintingPic:         INCBIN "gfx/paintings/lugia_painting.2bpp.lz"
BellTowerPaintingPic:     INCBIN "gfx/paintings/bell_tower_painting.2bpp.lz"
KabutoPuzzlePic:          INCBIN "gfx/paintings/kabuto_puzzle.2bpp.lz"
OmanytePuzzlePic:         INCBIN "gfx/paintings/omanyte_puzzle.2bpp.lz"
AerodactylPuzzlePic:      INCBIN "gfx/paintings/aerodactyl_puzzle.2bpp.lz"
HoOhPuzzlePic:            INCBIN "gfx/paintings/ho_oh_puzzle.2bpp.lz"
